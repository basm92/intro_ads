...
gradient_descent <- function(x, y, learn_rate, conv_threshold, n, max_iter) {
  ...
  while(converged == F) {
    ...
    if(MSE - MSE_new <= conv_threshold) {
      converged = T
      return(paste("Converged. Optimal intercept:", alpha, "Optimal slope:", beta1, beta2))
    }
    iterations = iterations + 1
    if(iterations > max_iter) { 
      converged = T
      return(paste("Optimal intercept:", alpha, "Optimal slope:", beta1, beta2))
    }
  }
}
```
...
```{r}
gradient_descent(cbind(data$educ, data$democracy), 
                 data$poverty, 
                 learn_rate = 0.0001,
...
$$
In the next few steps, we want to implement these functions in our gradient descent algorithm. We can largely use the same setup as we used in the linear model, with the exception of the derivates. 
...
```{r}
gradient_descent(cbind(data$educ, data$democracy), data$povyesno,
                 learn_rate = 0.001, 
...
GrowthDJ <- GrowthDJ %>%
  na.omit() %>%
  mutate(gdp60 = gdp60/10000, popgrowth = popgrowth/10, growth = if_else(gdpgrowth > median(gdpgrowth, na.rm = TRUE), 1, 0))
...
**Question x**: Verify that the solution from the analytical model (`lm(y ~ x1 + x2, data = datas)`) is identical to the solution you obtained when performing gradient descent. 
...
**Question x**: Verify that the solution for the logistic regression model is nearly identical to the solution from your gradient descent algorithm's output. Logistic regression in R can be done with `glm(data = data, povyesno ~ educ + democracy, link = "binomial")`. 

```{r}
glm(data = data, povyesno ~ educ + democracy, family="binomial") %>% summary()
